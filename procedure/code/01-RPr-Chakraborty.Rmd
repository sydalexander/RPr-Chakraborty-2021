...
filter(acs_covid, is.na(bpov_pct)) %>% st_drop_geometry() %>% 
  t() %>%
  kable(align = "c", caption = "Missing County Data") %>%
...
Here, we map the spatial distribution of the percent of people 75 or older to look for spatial patterns between old age and COVID-19 incidence, looking at one of the sub-groups within the wider categorization of "disability."

```{r map-age-75, message = FALSE}
tm_age_75 <- tm_shape(acs_covid) +
  tm_polygons("pct_75",
    title = "Percent of People Aged 75+\n(ACS 2014-2018)",
    style = "quantile",
    border.alpha = .2,
    lwd = 0.2,
    palette = "Oranges"
...
A map visualizing the percentage of people aged 75+ was also added. The two original maps investigate the relationship between COVID-19 incidence and disability.
However, disability is a broad category, made up of several different socio-demographic factors, and I visualized one of those categories. It could be worth exploring if there are clusters or groups of old people living together (eg. in retirement communities). 
Given the fact that older people are at higher risk of being affected by COVID-19, I chose to look at age, and in particular, those who were 75+.
For the future, it would be worthwhile to expand this category from 75+ to 65+, which would combine two of the colums from the original acs_covid data, as this would better represent the population at risk.
