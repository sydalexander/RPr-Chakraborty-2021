...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt, Sydney Alexander"
date: "`r Sys.Date()`"
...
```{r missing data table FIX}
#FIX - Missing Data Clean Table
#changed the Kable parameters in the original code block to improve the formatting of the table
# county with missing data
 
# WORKS
filter(acs_covid, is.na(bpov_pct)) %>% st_drop_geometry() %>% 
  t() %>%
  kable(col.names = c("Value"), align = "c", caption = "Missing County Data") %>%
...
**Unplanned deviation for reproduction** Visualizations for the original study and its reproduction are concerned with the relationship between COVID-19 incidence and with disability.
Here, we map the spatial distribution of the percent of people 65 or older to look for spatial patterns between old age and COVID-19 incidence, looking at one of the sub-groups within the wider categorization of "disability."

```{r map-age-65-75, message = FALSE}
#new column 65-75
acs_covid_MOD <- acs_covid %>%
  add_column(add_column = "pct_65_75")
acs_covid_MOD$pct_65_75 <- (acs_covid_MOD$pct_65_74 + acs_covid_MOD$pct_75)

tm_age_65_75 <- tm_shape(acs_covid_MOD) +
  tm_polygons("pct_65_75",
    title = "Percent of People Aged 65+ With a Disability\n(ACS 2014-2018)",
    style = "quantile",
    border.alpha = .2,
    lwd = 0.2,
    palette = "Oranges"
...
**Unplanned deviation** We reproduce the descriptive statistics, this time using the new age column for those aged 65+. 

```{r descriptive-statistics-with 65+}
acs_covid_stats_NEW <- acs_covid_MOD %>%
  st_drop_geometry() %>%
  select(covid_rate, contains("pct")) %>%
  stat.desc(norm = TRUE) %>%
  round(2) %>%
...
## COVID-19 Incidence Map and Summary statistic Reproductions
...
### Mapping COVID-19 Incidence and Disability Rates

In order to better understand the geographic patterns underlying the correlations between disability and COVID-19, we also visualized disability rates by county (Figure 3).
...
### Visualization of COVID-19 Clusters from SaTScan and Relative Risk
...
A cluster-based relative risk classification seemed more appropriate than a local county-based relative risk classification, by applying the same risk level to the entire COVID cluster and allowing the GEE models to analyze variance within the clusters.

### Cluster-based Relative Risk
...
Minor changes that improved reproducibility of the study included using the **kable** and **transposing** function to organize the table produced for the **missing data**, which included Rio Arriba County, New Mexico.

The choropleth map visualizing each county's weight in GEE model originally had an incorrect parameter which caused it to use use only half of the available color ramp (midpoint = 0). This parameter was removed to ensure that the map would use the entire color ramp. 

**Emily's discussion** was integrated into the discussion section, and a **Conclusions** section was separated out from the discussion section above. 

The **rationale for updated report** section was moved to the end of the paper. 

A map visualizing the percentage of people aged 65+ was also added. The two original maps investigate the relationship between COVID-19 incidence and disability.
However, disability is a broad category, made up of several different socio-demographic factors, and I visualized one of those categories.
Given the fact that older adults are at greater risk of being impacted by COVID-19, I combined those aged 65_74 with those 75+, as this new category would better represent a population who is at risk. 
Furthermore, the Pearson's correlation coefficients for both age groups in the original study are statistically significant when p<0.01, which indicates a negative correlation between COVID-19 incidence and these age groups. This is a surprising result; given that these are populations at higher risk of COVID-19, it seems counter intuitive that counties with greater percentages of older people would be experiencing smaller incidences of COVID-19.
It makes sense that the elderly with disability map correlates with the overall disability map given how common disabilities are with the elderly.
There are certain areas of the map including the Appalachia region, northern Michigan, as well as southern states including Missouri, Oklahoma, and Arkansas, where the elderly population with disabilities is fairly high that have a low spread of COVID-19. By the time Chakraborty conducted his study in 2018, it was likely that COVID-19 had not yet spread to these areas. Oppositely, there were high rates of COVID-19 in metropolitan areas with very little elderly disability, including the east coast Megalopolis, Miami, and in California (Holler). 
Comparing the COVID-19 incidence map and the Age 65+ Map, there does not seem to be much of a spatial relationship between COVID-19 incidence and old age. This conclusion could be verified by running a new Pearsons' r test. 
...
Although some of our results differed from Chakraborty's because of differences in analytical methods, conceptualizations, and computational environments, our reproduction results still suggest that PwDs are likely to experience multiple jeopardy based on the convergence of their disability, racial/ethnic minority, and poverty status.
